#!/usr/bin/env python
"""
Script để sửa dữ liệu khách hàng có province_id không hợp lệ
"""
import os
import sys
import django
...
def fix_customer_province_ward():
    """Sửa dữ liệu province và ward của khách hàng"""
    print("=== SỬA DỮ LIỆU KHÁCH HÀNG ===")
    
    # Lấy tỉnh Hà Nội
    try:
        hanoi = Province.objects.get(code='01')
        print(f"✅ Tỉnh Hà Nội: {hanoi.name} (ID: {hanoi.pk})")
    ...
    # Sửa tất cả khách hàng có province_id không hợp lệ
        ...
        if customer.ward_id:
            try:
                ...
                if first_ward:
                    ...
                    customer.ward_id = first_ward.pk
                    fixed = True
